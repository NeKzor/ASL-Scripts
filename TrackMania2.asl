...
startup
{
	...
	vars.TryInit = (Func<Process, ProcessModuleWow64Safe, bool>)((gameProc, module) =>
	{
		...
		// \x55\x8B\xEC\x83\xE4\xF8\x6A\xFF\x68\x00\x00\x00\x00\x64\xA1\x00\x00\x00\x00\x50\x83\xEC\x10\x53\x56\x57\xA1\x00\x00\x00\x00\x33\xC4\x50\x8D\x44\x24\x20\x64\xA3\x00\x00\x00\x00\x8B\xF1\x89\x74\x24\x18 xxxxxxxxx????xx????xxxxxxxx????xxxxxxxxx????xxxxxx
		var GameInfoTarget = new SigScanTarget(113, "55 8B EC 83 E4 F8 6A FF 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC 10 53 56 57 A1 ?? ?? ?? ?? 33 C4 50 8D 44 24 20 64 A3 ?? ?? ?? ?? 8B F1 89 74 24 18");
