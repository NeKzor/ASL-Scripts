...
startup
{
	...
	vars.TryParseTime = (Func<string, bool>)((timeValue) => vars.TimePattern.IsMatch(timeValue));
	vars.GetMap = (Func<string, Tuple<string, uint>>)((mapKey) =>
	{
		var map = default(Tuple<string, uint>);
		if (vars.Maps.TryGetValue(mapKey, out map))
			return map;
...
update
{
	...
	if (timer.CurrentPhase == TimerPhase.NotRunning)
	{
		...
		vars.CurrentMapName = vars.FirstMap = vars.GetFirstMap(vars.CategoryName);
	}
	if (vars.Init)
	{
		...
		{	
			if (!vars.TryParseTime(vars.Time.Old))
			{
				...
				{
					if (!vars.Time.Current.StartsWith("-"))
					{
